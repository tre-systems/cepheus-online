...
describe('room publication flow', () => {
  ...
  it('recovers finalized character creation from interval checkpoint plus tail', async () => {
    ...
    assert.equal(
      refreshedCharacter?.age,
      18,
      'non-owner refreshes should not reveal finalized age while its source aging dice are still hidden'
    )
    assert.deepEqual(
    ...
    assert.deepEqual(
      refreshedCharacter?.equipment,
      [],
      'non-owner refreshes should not reveal finalized equipment while its source mustering dice are still hidden'
    )
    assert.equal(
      refreshedCharacter?.credits,
      0,
      'non-owner refreshes should not reveal finalized credits while their source mustering dice are still hidden'
    )
    assert.equal(
